...
public class FileViewAction extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(file.getFiletype().equalsIgnoreCase("jpg") || file.getFiletype().equalsIgnoreCase("gif")|| file.getFiletype().equalsIgnoreCase("png") || file.getFiletype().equalsIgnoreCase("jpeg")) {
			response.setContentType("application/octet-stream;charset=utf-8");
			OutputStream os = response.getOutputStream();
			File f = new File("c://boardFile/",file.getSavedfile());
			...
			reader = new BufferedReader(new FileReader("c://boardFile/"+file.getSavedfile()));
			 while(true){
